...
export const acceptQuote = async (contractorId, quoteId, customerMessage = '') => {
    ...
    try {
        ...
        if (quote.status === 'accepted') throw new Error('Quote has already been accepted');
        ...
        return { success: true, jobId: jobRef.id, jobNumber, customerId };
    } catch (error) {
...
const createProConnection = async (userId, proData) => {
    try {
        ...
        if (!existing.empty) {
            await updateDoc(existing.docs[0].ref, { ...proData, updatedAt: serverTimestamp() });
        } else {
            await addDoc(prosRef, { ...proData, createdAt: serverTimestamp() });
        }
    } catch (error) {
