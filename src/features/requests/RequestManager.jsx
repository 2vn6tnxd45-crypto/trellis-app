...
import { Link as LinkIcon, Trash2, ArrowDownToLine, MapPin, Link2 } from 'lucide-react'; // Added MapPin, Link2
...
export const RequestManager = ({ userId, propertyName, propertyAddress, records, onRequestImport }) => {
    ...
    const handleCreateRequest = async (e) => {
        ...
        setIsCreating(true);
        try {
            // Find linked record details to snapshot
            ...
            await addDoc(collection(db, REQUESTS_COLLECTION_PATH), {
                ...
                linkedContext: linkedRecord ? {
                    item: linkedRecord.item,
                    brand: linkedRecord.brand,
                    model: linkedRecord.model,
                    year: linkedRecord.dateInstalled ? new Date(linkedRecord.dateInstalled).getFullYear() : 'Unknown'
                } : null,
    ...
    return (
        <div className="space-y-8">
            <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-sky-100">
                ...
                <p className="text-slate-500 mb-6">Generate a secure link to send to your contractor. You can optionally share your address or current equipment details.</p>
                ...
                <form onSubmit={handleCreateRequest} className="space-y-4">
                    <input 
                        ...
                        placeholder="Project Description (e.g. Replace HVAC)" 
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        <div className="flex-1 bg-slate-50 p-3 rounded-xl border border-slate-100 flex items-center">
                             <input 
                                ...
                                className="mr-3 h-5 w-5 rounded text-sky-600 focus:ring-sky-500"
                             />
                             <label htmlFor="shareAddr" className="text-sm font-bold text-slate-700 cursor-pointer flex items-center">
                                <MapPin size={16} className="mr-2 text-slate-400"/> Share Property Address
                             </label>
                        </div>
                        ...
                        <div className="flex-1 bg-slate-50 p-3 rounded-xl border border-slate-100 flex items-center">
                             ...
                             >
                                ...
                                {records && records.map(r => (
                                    <option key={r.id} value={r.id}>{r.item} ({r.brand || 'Generic'})</option>
                                ))}
                             </select>
                        </div>
                    </div>

                    <button type="submit" disabled={isCreating || !newRequestDesc} className="w-full bg-sky-900 text-white py-4 rounded-xl font-bold hover:bg-sky-800 disabled:opacity-50">
                    ...
                    <div key={req.id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
                        ...
                        <div className="flex gap-2 w-full md:w-auto">
                            {req.status === 'submitted' && (
                                <button onClick={() => onRequestImport(req)} className="flex-1 md:flex-none flex items-center justify-center px-4 py-2 bg-green-50 text-green-700 rounded-lg font-bold text-sm hover:bg-green-100 transition-colors border border-green-200">
