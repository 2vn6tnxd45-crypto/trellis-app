        ...
        <div className="space-y-8">
            <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-sky-100">
                ...
                <form onSubmit={handleCreateRequest} className="space-y-4">
                    <input type="text" value={newRequestDesc} onChange={(e) => setNewRequestDesc(e.target.value)} placeholder="Project Description (e.g. Replace HVAC)" className="w-full p-4 rounded-xl border border-slate-200 focus:ring-2 focus:ring-sky-500 outline-none"/>
                    
                    <div className="flex flex-col sm:flex-row gap-4">
                        <div className="flex-1 bg-slate-50 p-3 rounded-xl border border-slate-100 flex items-center">
                             <input type="checkbox" id="shareAddr" checked={shareAddress} onChange={(e) => setShareAddress(e.target.checked)} className="mr-3 h-5 w-5 rounded text-sky-600 focus:ring-sky-500"/>
                    ...
                    <button type="submit" disabled={isCreating || !newRequestDesc} className="w-full bg-sky-900 text-white py-4 rounded-xl font-bold hover:bg-sky-800 disabled:opacity-50">{isCreating ? 'Creating...' : 'Create Secure Link'}</button>
                </form>
            </div>
