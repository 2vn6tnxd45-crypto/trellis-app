    ...
    return (
        <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl shadow-2xl p-6 z-20 animate-slide-up">
            <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={onChangeType} className="text-sm text-slate-500 hover:text-slate-700 font-medium">
                    Change type
                </button>
            </div>
            ...
            <div className="flex gap-3">
                <button onClick={onRetry} className="flex-1 py-4 border-2 border-slate-200 rounded-2xl font-bold text-slate-700 hover:bg-slate-50 transition-colors flex items-center justify-center gap-2">
                    <RotateCcw className="h-5 w-5" /> Retake
                </button>
                <button onClick={onConfirm} className="flex-[2] py-4 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-2xl font-bold hover:shadow-lg hover:shadow-emerald-500/30 transition-all flex items-center justify-center gap-2">
                    <Sparkles className="h-5 w-5" /> Extract Details
                </button>
            </div>
        </div>
    );
...
export const SmartScanner = ({ 
    ...
    // Mode: 'select' (default), 'camera', 'upload'
    ...
    return (
        <div className="fixed inset-0 bg-black z-50 flex flex-col">
            ...
            <div className="absolute top-0 left-0 right-0 z-30 p-4 flex items-center justify-between bg-gradient-to-b from-black/80 to-transparent">
                <button onClick={onClose} className="p-3 bg-white/10 backdrop-blur-sm rounded-full hover:bg-white/20 transition-colors">
                ...
                {mode === 'select' && (
                    <div className="w-full max-w-md p-6 space-y-6 animate-in fade-in zoom-in-95 duration-300">
                        <div className="text-center space-y-2 mb-8">
                            ...
                            <p className="text-slate-400">Scan receipts, labels, or entire rooms.</p>
                        </div>

                        <button onClick={() => setMode('camera')} className="w-full p-6 bg-emerald-600 hover:bg-emerald-500 rounded-3xl flex items-center gap-6 transition-all group shadow-lg shadow-emerald-900/20">
                            <div className="bg-white/20 p-4 rounded-2xl group-hover:scale-110 transition-transform">
                                <Camera className="h-8 w-8 text-white" />
                            </div>
                            <div className="text-left">
                                <h3 className="font-bold text-xl text-white">Use Camera</h3>
                                <p className="text-emerald-100/80 text-sm">Snap a photo directly</p>
                            </div>
                        </button>

                        <button onClick={() => fileInputRef.current?.click()} className="w-full p-6 bg-slate-800 hover:bg-slate-700 rounded-3xl flex items-center gap-6 transition-all group border border-slate-700">
                            <div className="bg-slate-700 p-4 rounded-2xl group-hover:bg-slate-600 transition-colors border border-slate-600">
                                <ImageIcon className="h-8 w-8 text-white" />
                            </div>
                            <div className="text-left">
                                <h3 className="font-bold text-xl text-white">Upload File</h3>
                                <p className="text-slate-400 text-sm">Use existing photo or PDF</p>
                            </div>
                        </button>
                    </div>
                )}
            ...
            {mode === 'camera' && !capturedImage && !scanStage && (
                <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-black/80 to-transparent">
                    <div className="flex items-center justify-center gap-8">
                        <button onClick={() => setMode('select')} className="p-4 bg-white/10 backdrop-blur-sm rounded-full hover:bg-white/20 transition-colors">
                            <X className="h-6 w-6 text-white" />
                        </button>
                        <button onClick={capturePhoto} className="w-20 h-20 bg-white rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-transform shadow-2xl border-4 border-slate-900/20">
            ...
            {detectedType && !scanStage && !showTypeSelector && (
                <DetectionResult type={detectedType} confidence={confidence} onConfirm={handleConfirm} onRetry={handleRetry} onChangeType={() => setShowTypeSelector(true)} />
            )}
            
            {/* TYPE SELECTOR */}
            {showTypeSelector && <TypeSelector onSelect={handleTypeSelect} onClose={() => setShowTypeSelector(false)} />}
