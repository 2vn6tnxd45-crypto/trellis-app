...
import { Wrench, CheckCircle, Zap, Plus } from 'lucide-react';
...
export const MaintenanceDashboard = ({ records, onCompleteTask, onAddStandardTask }) => {
    ...
    useEffect(() => {
        if (records) {
            // 1. Calculate Status for existing records
            ...
            // 2. Identify Missing Standard Tasks
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {suggestions.map((suggestion, idx) => (
                            <div key={idx} className="bg-white p-4 rounded-2xl border border-sky-100 shadow-sm flex justify-between items-center hover:border-sky-300 transition-colors group">
                                ...
                                <button onClick={() => onAddStandardTask(suggestion)} className="p-2 bg-sky-100 text-sky-700 rounded-full hover:bg-sky-600 hover:text-white transition shadow-sm" title="Add to Log">
                                    <Plus size={18} />
                                </button>
                            </div>
                        ))}
