...
import { ChevronDown, Zap, Wrench, Camera, Pencil, PlusCircle, X, ChevronUp, ChevronRight } from 'lucide-react'; // Added ChevronUp, ChevronRight
...
export const AddRecordForm = ({ onSave, onBatchSave, isSaving, newRecord, onInputChange, onFileChange, fileInputRef, isEditing, onCancelEdit }) => {
    ...
    // Default to expanded if we are editing, otherwise collapsed (Quick Mode)
    ...
    const handleAutoFill = (data) => {
        ...
        setIsExpanded(true); // Auto-expand to show scanned details
    };
    ...
    return (
        <div className="bg-white rounded-[2rem] shadow-xl border border-slate-100 overflow-hidden">
            {/* Smart Scan - Only show in Add Mode */}
            ...
            <form onSubmit={onSave} className="p-10 pt-6 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="item" value={newRecord.item} onChange={onInputChange} required placeholder="e.g. North Wall" className="block w-full rounded-xl border-slate-200 bg-slate-50 p-3.5 border focus:ring-sky-500 focus:bg-white"/>
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Date Installed</label>
                        <input type="date" name="dateInstalled" value={newRecord.dateInstalled} onChange={onInputChange} className="block w-full rounded-xl border-slate-200 bg-slate-50 p-3.5 border focus:ring-sky-500"/>
                    </div>
                </div>

                {/* File Upload (Always visible for Quick Add) */}
                <div>
                    <div className="bg-slate-50 p-4 rounded-2xl border border-dashed border-slate-300 hover:border-sky-400 hover:bg-sky-50 transition-colors text-center cursor-pointer relative group flex flex-col items-center justify-center min-h-[100px]">
                        <label className="block text-sm font-bold text-slate-600 mb-1 group-hover:text-sky-700 pointer-events-none">
                            {newRecord.imageUrl || fileInputRef?.current?.files?.[0] ? "Change Photo" : "Add Photo / Receipt"}
                        </label>
                        ...
                        <div className="text-slate-400 group-hover:text-sky-400"><Camera size={20} className="mx-auto"/></div>
                    </div>
                    {newRecord.imageUrl && <div className="mt-2 p-2 bg-sky-50 rounded-xl border border-sky-100"><img src={newRecord.imageUrl} alt="Preview" className="h-24 rounded-lg object-cover"/></div>}
                </div>
                ...
                <button 
                    ...
                    className="flex items-center text-sm font-bold text-sky-600 hover:text-sky-800 transition-colors w-full justify-center py-2 bg-sky-50/50 rounded-lg hover:bg-sky-50"
                >
                    {isExpanded ? (
                        <><ChevronUp size={16} className="mr-1"/> Hide Details</>
                    ) : (
                        <><ChevronRight size={16} className="mr-1"/> Add Details, Specs & Maintenance</>
                    )}
                </button>
