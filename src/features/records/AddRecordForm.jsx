                            ...
                            <div key={idx} className={`flex flex-col gap-2 p-4 border rounded-xl items-start ${duplicate ? 'bg-amber-50 border-amber-200' : 'bg-white border-slate-200'}`}>
                                <div className="flex flex-col md:flex-row gap-4 w-full">
                                    ...
                                    <div className="w-full md:w-1/4">
                                        ...
                                        <Select
                                            value={item.category}
                                            onChange={(val) => updateBatchItem(idx, 'category', val)}
                                            options={[
                                                { value: '', label: 'Select...' },
                                    ...
                                    <div className="w-full md:w-1/4">
                                        ...
                                        <RoomSelector
                                            value={item.area}
                                            onChange={(val) => updateBatchItem(idx, 'area', val)}
                                            allowCustom={true}
                                        />
                                    </div>
        ...
        <div className="bg-white rounded-[2rem] shadow-xl border border-slate-100 overflow-hidden flex flex-col max-h-[85vh]">
            ...
            <div className="overflow-y-auto p-6 pt-2 flex-grow">
                <form id="recordForm" onSubmit={onSave} className="space-y-6">
                    ...
                    {(step === 2 || isEditing) && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Area *</label>
                                    <RoomSelector
                                        value={newRecord.area}
                                        onChange={(val) => onInputChange({ target: { name: 'area', value: val } })}
                                    />
                                </div>
                            </div>
                            <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Date Installed</label><input type="date" name="dateInstalled" value={newRecord.dateInstalled} max={new Date().toISOString().split('T')[0]} onChange={onInputChange} className="block w-full rounded-xl border-slate-200 bg-white p-3 border focus:ring-emerald-500 text-sm text-slate-600" /></div>
                    ...
                    {(step === 3 || isEditing) && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                            <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 space-y-4"><h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center"><Tag size={12} className="mr-1" /> Product Specs</h4><div className="grid grid-cols-2 gap-4"><div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Brand</label><input type="text" name="brand" value={newRecord.brand} onChange={onInputChange} maxLength={255} placeholder="e.g. Samsung" className="block w-full rounded-lg border-slate-200 p-2.5 border text-sm bg-white" /></div><div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Model</label><input type="text" name="model" value={newRecord.model} onChange={onInputChange} maxLength={255} placeholder="Model #" className="block w-full rounded-lg border-slate-200 p-2.5 border text-sm bg-white" /></div></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Cost ($)</label><input type="number" name="cost" value={newRecord.cost} onChange={onInputChange} min="0" onBlur={(e) => { if (parseFloat(e.target.value) < 0) onInputChange({ target: { name: 'cost', value: Math.abs(parseFloat(e.target.value)) } }) }} placeholder="0.00" className="block w-full rounded-lg border-slate-200 p-2.5 border text-sm bg-white" /></div><div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Contractor</label><input type="text" name="contractor" value={newRecord.contractor} onChange={onInputChange} maxLength={255} placeholder="Company Name" className="block w-full rounded-lg border-slate-200 p-2.5 border text-sm bg-white" /></div></div></div>
                            ...
                            <div className="bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100 space-y-3">
                                ...
                                <div>
                                    <Select
                                        value={newRecord.maintenanceFrequency}
                                        onChange={(val) => onInputChange({ target: { name: 'maintenanceFrequency', value: val } })}
                            ...
                            <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Notes</label><textarea name="notes" value={newRecord.notes} onChange={onInputChange} rows={3} maxLength={1000} className="block w-full rounded-xl border-slate-200 bg-white p-3 border focus:ring-emerald-500 text-sm resize-none" placeholder="Add details..."></textarea></div>
