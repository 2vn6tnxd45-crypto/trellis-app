...
const calculateNextDate = (dateInstalled, frequencyValue) => {
    if (!dateInstalled || !frequencyValue || frequencyValue === 'none') return null;
    const freq = MAINTENANCE_FREQUENCIES.find(f => f.value === frequencyValue);
    if (!freq) return null;
    
    const date = new Date(dateInstalled);
    // Simple month addition (handles year rollover automatically)
...
const initialRecordState = {
    ...
    notes: '', purchaseLink: '', imageUrl: '', maintenanceFrequency: 'none', nextServiceDate: null,
};
...
const MaintenanceDashboard = ({ records }) => {
    const tasks = records
        .filter(r => r.nextServiceDate && r.maintenanceFrequency !== 'none')
    ...
    const getStatus = (dateStr) => {
        ...
        if (diffDays < 0) return { label: 'Overdue', color: 'text-red-700 bg-red-50 border-red-200', days: `${Math.abs(diffDays)} days overdue`, icon: AlertTriangle };
        if (diffDays <= 30) return { label: 'Due Soon', color: 'text-yellow-700 bg-yellow-50 border-yellow-200', days: `Due in ${diffDays} days`, icon: Clock };
        return { label: 'Upcoming', color: 'text-green-700 bg-green-50 border-green-200', days: `Due in ${diffDays} days`, icon: Calendar };
    };

    return (
        <div className="space-y-8">
            <div className="bg-blue-50 p-6 rounded-xl border border-blue-100">
                <h3 className="text-lg font-bold text-blue-900">Maintenance Schedule</h3>
                <p className="text-sm text-blue-600">Upcoming tasks based on your installation dates and maintenance frequencies.</p>
            </div>
            ...
            {tasks.length === 0 ? (
                <div className="p-12 text-center border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50">
                    ...
                    <p className="text-gray-500 font-medium">No scheduled maintenance tasks.</p>
                    <p className="text-xs text-gray-400">Add a record with a maintenance frequency to see it here.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 gap-4">
                    {tasks.map(task => {
                        const status = getStatus(task.nextServiceDate);
                        const StatusIcon = status.icon;
                        return (
                            <div key={task.id} className={`flex items-center justify-between p-4 rounded-xl border ${status.color} transition hover:shadow-sm`}>
                                <div className="flex items-center">
                                    <div className={`p-2 rounded-full bg-white mr-4 border border-current opacity-80`}>
                                        <StatusIcon className="h-5 w-5"/>
                                    </div>
                                    <div>
                                        <h4 className="font-bold text-gray-900">{task.item}</h4>
                                        <p className="text-xs font-medium opacity-90">{task.category} • {status.days}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <span className="block text-sm font-bold">{new Date(task.nextServiceDate).toLocaleDateString()}</span>
                                    <span className="text-xs uppercase tracking-wider font-semibold opacity-75">{status.label}</span>
                                </div>
                            </div>
                        );
                    })}
...
const ContractorView = () => {
    ...
    const [formData, setFormData] = useState({ category: '', item: '', brand: '', model: '', notes: '', contractor: '', maintenanceFrequency: 'none' });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Calculate next service date if maintenance is set
            let nextServiceDate = null;
            if (formData.maintenanceFrequency !== 'none') {
                // Contractors usually submit for work done *today* or recently, assume today for calculation base
                nextServiceDate = calculateNextDate(new Date().toISOString().split('T')[0], formData.maintenanceFrequency);
            }
...
const RequestManager = ({ userId, propertyName }) => {
    ...
    const approveRequest = async (req) => {
        ...
        try {
            await addDoc(collection(db, `/artifacts/${appId}/public/data/house_records`), {
                ...
                dateInstalled: req.dateInstalled || new Date().toISOString().split('T')[0], 
    ...
    const sendEmail = (id, description) => {
        const baseUrl = window.location.href.split('?')[0];
        const url = `${baseUrl}?requestId=${id}`;
        const subject = encodeURIComponent(`HausKey Request: ${description}`);
...
// ... [EnvironmentalInsights, PropertyMap, PedigreeReport, RecordCard remain same] ...
...
const AddRecordForm = ({ onSave, isSaving, newRecord, onInputChange, onFileChange, fileInputRef, isEditing, onCancelEdit }) => {
    ...
    return (
        <form onSubmit={onSave} className="p-6 bg-white rounded-xl shadow-2xl border-t-4 border-indigo-600 space-y-4">
            ...
            <div><label className="block text-sm font-medium text-gray-700">Maintenance Frequency</label><div className="relative mt-1"><select name="maintenanceFrequency" value={safeRecord.maintenanceFrequency} onChange={onInputChange} className="block w-full rounded-lg border-gray-300 shadow-sm p-2 border appearance-none">{MAINTENANCE_FREQUENCIES.map(f=><option key={f.value} value={f.value}>{f.label}</option>)}</select><ChevronDown size={16} className="absolute right-2 top-3 text-gray-500 pointer-events-none"/></div></div>
