...
import { Trash2, PlusCircle, Home, Calendar, PaintBucket, HardHat, Info, FileText, ExternalLink, Camera, MapPin, Search, LogOut, Lock, Mail, ChevronDown, Hash, Layers, X, Printer, Map as MapIcon, ShoppingBag, Sun, Wind, Zap, AlertTriangle, UserMinus, Pencil, Send, CheckCircle, Link as LinkIcon, Clock } from 'lucide-react';
...
const REQUESTS_COLLECTION_PATH = `/artifacts/${appId}/public/data/requests`; // New Collection for Contractor Requests
...
// ... [Previous Auth & Setup Components - Kept same as before] ...
...
const ContractorView = () => {
    ...
    useEffect(() => {
        ...
        // Using signInAnonymously to allow write to specific doc via security rules
        // For now, we just fetch the doc. Real security rules would check the ID.
        const fetchRequest = async () => {
            ...
            } catch (e) { setError("Could not load request."); console.error(e); } finally { setLoading(false); }
        };
        fetchRequest();
    }, []);
    ...
    if (submitted) return <div className="min-h-screen flex items-center justify-center bg-green-50 p-8 text-center"><div><CheckCircle className="h-16 w-16 text-green-600 mx-auto mb-4"/><h1 className="text-2xl font-bold text-green-800">Submission Received!</h1><p className="text-green-600">The homeowner has been notified.</p></div></div>;
    if (error) return <div className="min-h-screen flex items-center justify-center bg-red-50 p-8 text-center text-red-700">{error}</div>;

    return (
        <div className="min-h-screen bg-gray-50 p-6 font-sans">
            <div className="max-w-md mx-auto bg-white rounded-xl shadow-lg p-8 border-t-4 border-indigo-600">
                <h1 className="text-2xl font-bold text-indigo-900 mb-2">Contractor Submission</h1>
                <p className="text-gray-600 mb-6 text-sm">Please fill out the details for the work performed at <strong>{requestData.propertyName}</strong>.</p>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <input type="text" placeholder="Your Name / Company" className="w-full p-3 border rounded-lg" required value={formData.contractor} onChange={e=>setFormData({...formData, contractor: e.target.value})} />
                    <select className="w-full p-3 border rounded-lg" required value={formData.category} onChange={e=>setFormData({...formData, category: e.target.value})}><option value="" disabled>Select Category</option>{CATEGORIES.map(c=><option key={c} value={c}>{c}</option>)}</select>
                    <input type="text" placeholder="Item Name (e.g. New Furnace)" className="w-full p-3 border rounded-lg" required value={formData.item} onChange={e=>setFormData({...formData, item: e.target.value})} />
                    <div className="grid grid-cols-2 gap-4">
                        <input type="text" placeholder="Brand" className="w-full p-3 border rounded-lg" value={formData.brand} onChange={e=>setFormData({...formData, brand: e.target.value})} />
                        <input type="text" placeholder="Model" className="w-full p-3 border rounded-lg" value={formData.model} onChange={e=>setFormData({...formData, model: e.target.value})} />
                    </div>
                    <textarea placeholder="Notes / Warranty Info" className="w-full p-3 border rounded-lg h-24" value={formData.notes} onChange={e=>setFormData({...formData, notes: e.target.value})}></textarea>
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                        <label className="cursor-pointer text-indigo-600 font-medium flex flex-col items-center"><Camera className="mb-1"/> Upload Receipt/Photo <input type="file" className="hidden" accept="image/*" onChange={e=>setSelectedFile(e.target.files[0])}/></label>
                        {selectedFile && <p className="text-xs text-gray-500 mt-2">{selectedFile.name}</p>}
                    </div>
                    <button type="submit" className="w-full bg-indigo-600 text-white font-bold py-3 rounded-lg hover:bg-indigo-700 transition">Submit Record</button>
                </form>
            </div>
        </div>
    );
...
// NEW: Request Manager Component (For Homeowner)
const RequestManager = ({ userId, propertyName }) => {
    ...
    const copyLink = (id) => {
        const url = `${window.location.origin}/app?requestId=${id}`;
...
const AppContent = () => {
    ...
    useEffect(() => {
        ...
        if (params.get('requestId')) {
            ...
            // In contractor mode, we don't need standard auth check, just firebase init
        }
        ...
        if (firebaseConfig) {
            try {
                ...
                if (!params.get('requestId')) {
                    ...
                    setLoading(false); // Stop global loader for contractor
                }
    ...
    // ... (Handlers for Login, Save, Edit - Same as before)
    ...
    // RENDER LOGIC
    if (isContractorMode) return <ContractorView />; // Route to Contractor View if URL param exists
