...
// Contractor Form Component (UPDATED: Better Formatting)
const ContractorView = () => {
    ...
    useEffect(() => {
        ...
        const fetchRequest = async () => {
            try {
                if (!auth.currentUser) { 
                    try { await signInAnonymously(auth); } catch(e) { /* ignore anon auth fails if restricted */ } 
                }
    ...
    if (submitted) return (
        <div className="min-h-screen flex items-center justify-center bg-green-50 p-8">
            <div className="bg-white p-10 rounded-2xl shadow-xl text-center max-w-md border-t-8 border-green-500">
                ...
                <p className="text-gray-600 text-lg">The homeowner has been notified and the record is ready for approval.</p>
            </div>
        </div>
    );
        ...
        <div className="min-h-screen bg-gray-50 p-6 font-sans flex items-center justify-center">
            <div className="max-w-2xl w-full bg-white rounded-2xl shadow-2xl border-t-8 border-indigo-600 overflow-hidden">
                <div className="bg-indigo-50 p-8 border-b border-indigo-100 text-center">
                     <img src={logoSrc} alt="Trellis Logo" className="h-16 w-16 mx-auto mb-4 shadow-sm rounded-xl bg-white p-2" />
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <input type="text" placeholder="e.g. Trane XV80 Furnace" className="w-full pl-10 p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" required value={formData.item} onChange={e=>setFormData({...formData, item: e.target.value})} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea placeholder="Enter installation details, warranty expiration, filter sizes, etc." className="w-full p-3 border border-gray-300 rounded-lg h-32 focus:ring-2 focus:ring-indigo-500 outline-none resize-none" value={formData.notes} onChange={e=>setFormData({...formData, notes: e.target.value})}></textarea>
                    </div>
