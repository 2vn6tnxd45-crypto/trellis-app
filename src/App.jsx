...
import { Trash2, PlusCircle, Home, Calendar, PaintBucket, HardHat, Info, FileText, ExternalLink, Camera, MapPin, Search, LogOut, Lock, Mail, ChevronDown, Hash, Layers, X, Printer, Map as MapIcon, ShoppingBag, Sun, Wind, Zap, AlertTriangle, UserMinus, Pencil, Send, CheckCircle, Link as LinkIcon, Clock, Palette, Key, User, Tag, Box, UploadCloud, Wrench, ListChecks, Plus, Sparkles, TrendingUp, ShieldCheck } from 'lucide-react';
        ...
        <div className="fixed inset-0 bg-sky-950/80 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
            <div className="bg-white rounded-[2rem] shadow-2xl max-w-lg w-full overflow-hidden border border-sky-100">
                ...
                <div className="p-8 space-y-6">
                    ...
                    <button 
                        onClick={onClose}
                        className="w-full py-4 bg-sky-900 text-white font-bold rounded-2xl shadow-lg shadow-sky-900/20 hover:bg-sky-800 hover:scale-[1.01] transition-all transform mt-4"
                    >
                        Let's Get Started
                    </button>
                </div>
...
const RecordCard = ({ record, onDeleteClick, onEditClick }) => ( <div className="bg-f8fafc border border-f1f5f9 rounded-[1.5rem] hover:bg-white hover:shadow-xl hover:-translate-y-0.5 hover:border-e2e8f0 transition-all duration-300 p-8 cursor-pointer group bg-slate-50 border-slate-100 relative overflow-hidden"> {record.imageUrl && <div className="h-48 w-full bg-gray-100 relative group print:h-32 rounded-t-[1.5rem] -mt-8 -mx-8 mb-6"><img src={record.imageUrl} alt={record.item} className="w-full h-full object-cover"/></div>} <div className="flex justify-between items-start mb-6"> <div className="p-2.5 rounded-full bg-slate-200 text-slate-500 group-hover:bg-sky-900 group-hover:text-white transition-colors duration-300"> <Home size={20} /> </div> <span className="bg-white border border-slate-200 text-slate-500 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">{String(record.category || 'General')}</span> </div> <div className="mb-4"> <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-1 group-hover:text-sky-400 transition-colors">Item</p> <h2 className="text-2xl font-bold text-slate-800 group-hover:text-slate-900 leading-tight">{String(record.item || 'Unknown')}</h2> </div> <div className="space-y-2 mb-6"> {record.brand && <p className="text-slate-500 text-sm flex items-center"><span className="w-1.5 h-1.5 rounded-full bg-slate-300 mr-2"></span>{record.brand}</p>} {record.dateInstalled && <p className="text-slate-500 text-sm flex items-center"><span className="w-1.5 h-1.5 rounded-full bg-slate-300 mr-2"></span>{record.dateInstalled}</p>} </div> <div className="flex justify-between items-center border-t border-slate-100 pt-4 mt-auto"> <div className="flex gap-2"> <button onClick={() => onEditClick(record)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 hover:text-sky-600 transition"><Pencil size={16}/></button> <button onClick={() => onDeleteClick(record.id)} className="p-2 hover:bg-red-50 rounded-full text-slate-400 hover:text-red-500 transition"><Trash2 size={16}/></button> </div> {record.maintenanceFrequency !== 'none' && <span className="text-xs font-bold text-sky-600 bg-sky-50 px-2 py-1 rounded-md">{MAINTENANCE_FREQUENCIES.find(f=>f.value===record.maintenanceFrequency)?.label}</span>} </div> </div> );
