...
// ============================================
// Sends a review request email to a customer after job completion
// Called by the cron job or manually via API
...
export default async function handler(req, res) {
    ...
    if (!jobId || !contractorId) {
        return res.status(400).json({ error: 'jobId and contractorId are required' });
    }

    console.log(`[SendReviewRequest] Processing job ${jobId} for contractor ${contractorId}`);

    try {
        // Fetch job data
        const appId = process.env.FIREBASE_APP_ID || 'krib-app';
        const jobRef = db.collection(`artifacts/${appId}/requests`).doc(jobId);
        const jobSnap = await jobRef.get();

        if (!jobSnap.exists) {
            console.log(`[SendReviewRequest] Job ${jobId} not found`);
        ...
        if (job.reviewRequestSent) {
            console.log(`[SendReviewRequest] Review request already sent for job ${jobId}`);
        ...
        if (!contractorSnap.exists) {
            console.log(`[SendReviewRequest] Contractor ${contractorId} not found`);
        ...
        if (!reviewSettings.autoRequestReviews) {
            console.log(`[SendReviewRequest] Auto-request disabled for contractor ${contractorId}`);
            return res.status(200).json({ success: true, disabled: true });
        }

        // Check if Google URL is configured
        if (!reviewSettings.googleBusinessUrl) {
            console.log(`[SendReviewRequest] No Google URL configured for contractor ${contractorId}`);
            return res.status(200).json({ success: true, noUrl: true });
        }

        // Get customer email
        const customerEmail = job.customerEmail || job.customer?.email;
        if (!customerEmail) {
            console.log(`[SendReviewRequest] No customer email for job ${jobId}`);
            return res.status(400).json({ error: 'No customer email' });
        }

        // Format completed date
        let completedDate = null;
        if (job.completion?.approvedAt) {
            const approvedAt = job.completion.approvedAt.toDate
                ? job.completion.approvedAt.toDate()
                : new Date(job.completion.approvedAt);
            completedDate = approvedAt.toLocaleDateString('en-US', {
                month: 'long',
                day: 'numeric',
                year: 'numeric'
            });
        ...
        console.log(`[SendReviewRequest] Review request sent for job ${jobId}, email ID: ${data.id}`);
